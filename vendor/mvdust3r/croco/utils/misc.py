...
class NativeScalerWithGradNormCount:
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True, model = None): # loss_scaler(loss, optimizer, parameters=model.parameters(), update_grad=(data_iter_step + 1) % accum_iter == 0)
        ...
        if update_grad:
            ...
            # grad_ma, param_ma = -1, -1
            # for name, param in model.named_parameters():
            #     if param.grad is not None:
            #         print('cgcgcg', name, 'check grad', param.grad.norm(), param.norm().item())
            #         grad_ma = max(grad_ma, param.grad.norm().item())
            #         param_ma = max(param_ma, param.norm().item())
            # print('optim ma', grad_ma, param_ma)
            self._scaler.step(optimizer)
            # param_ma = -1
            # for name, param in model.named_parameters():
            #     if param.grad is not None:
            #         print('cgcgcg after', name, param.norm().item())
            #         param_ma = max(param_ma, param.norm().item())
            # print('optim ma after', param_ma)
