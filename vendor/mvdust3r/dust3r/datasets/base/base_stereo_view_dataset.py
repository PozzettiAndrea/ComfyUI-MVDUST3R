...
class BaseStereoViewDataset (EasyDataset):
    ...
    def __getitem_bsvd__(self, idx, ref_view_id):

        return self.__getitem__(idx, ref_view_id)

    def __getitem__(self, idx, ref_view_id = None):
        self.cnt_all += 1
        ...
        views = self._get_views(idx, resolution, self._rng, ref_view_id = ref_view_id)
        ...
        # self.t_all += time.time()
        # print('dataset collection time', self.t_all, self.t_all / self.cnt_all) # trainer/0 [1]:dataset collection time 8.565221548080444 1.0706526935100555
        return views
