...
def fast_pnp(pts3d, focal, msk, device, pp=None, niter_PnP=10):
    ...
    for focal in tentative_focals:
        ...
        # print(dir(cv2)) # 'SOLVEPNP_AP3P', 'SOLVEPNP_DLS', 'SOLVEPNP_EPNP', 'SOLVEPNP_IPPE', 'SOLVEPNP_IPPE_SQUARE', 'SOLVEPNP_ITERATIVE', 'SOLVEPNP_MAX_COUNT', 'SOLVEPNP_P3P', 'SOLVEPNP_UPNP', 'SORT_ASCENDING', 'SORT_DESCENDING', 'SORT_EVERY_COLUMN', 'SORT_EVERY_ROW', 'SPARSE_MAT_HASH_BIT', 'SPARSE_MAT_HASH_SCALE'
        ...
        success, R, T, inliers = cv2.solvePnPRansac(pts3d[msk], pixels[msk], K, None,
                                                    iterationsCount=niter_PnP, reprojectionError=5, flags=cv2.SOLVEPNP_EPNP) 
        if not success:
            continue
