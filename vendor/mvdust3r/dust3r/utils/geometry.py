...
def depthmap_to_absolute_camera_coordinates(depthmap, camera_intrinsics, camera_pose, **kw): # camera_pose: c2w
    """
    Args:
        - depthmap (HxW array):
        - camera_intrinsics: a 3x3 matrix
...
def normalize_pointclouds(pts1, pts2s, norm_mode='avg_dis', valid1=None, valid2s=None, return_norm_factor = False):
    """ renorm pointmaps pts1, pts2 with norm_mode
    """
    assert pts1.ndim >= 3 and pts1.shape[-1] == 3
    ...
    if norm_mode == 'avg':
        ...
        nan_pts2s, nnz2s = zip(*nan_pts2_nnz2s)
        nan_pts2s = list(nan_pts2s)
        nnz2s = list(nnz2s)
        all_pts = torch.cat((nan_pts1, *nan_pts2s), dim=1) if pts2s[0] is not None else nan_pts1
        ...
        if dis_mode == 'dis':
            ...
            raise ValueError(f'bad {dis_mode=}')
        # print('nnz2s', nnz1, nnz2s, all_dis.shape, pts1.shape, [pts2.shape for pts2 in pts2s]) # trainer/0 [0]:[17:51:36.116105] nnz2s tensor([48653, 49755], device='cuda:0') [tensor([48203, 49610], device='cuda:0'), tensor([48614, 46611], device='cuda:0'), tensor([46879, 47230], device='cuda:0')] torch.Size([2, 200704]) torch.Size([2, 224, 224, 3]) [torch.Size([2, 224, 224, 3]), torch.Size([2, 224, 224, 3]), torch.Size([2, 224, 224, 3])]
        norm_factor = all_dis.sum(dim=1) / (nnz1 + sum(nnz2s) + 1e-8) # a bug fixed here, need re-run MV experiments, also check why MV experiments stop in the middle.
    else:
        raise NotImplementedError()

    norm_factor = norm_factor.clip(min=1e-8)
    norm_factor_original = norm_factor.clone()
    while norm_factor.ndim < pts1.ndim:
        norm_factor.unsqueeze_(-1)

    res = pts1 / norm_factor
    if pts2s is not None:
        if return_norm_factor:
            res = res, [pts2 / norm_factor for pts2 in pts2s], norm_factor_original
        else:
            res = res, [pts2 / norm_factor for pts2 in pts2s]    
    return res
...
def get_joint_pointcloud_depths(z1, z2s, valid_mask1, valid_mask2s=None, quantile=0.5):
    # set invalid points to NaN
    _z1 = invalid_to_nans(z1, valid_mask1).reshape(len(z1), -1)
...
@torch.no_grad()
def get_joint_pointcloud_center_scales(pts1, pts2s, valid_mask1=None, valid_mask2s=None, z_only=False, center=True):
    # set invalid points to NaN
    _pts1 = invalid_to_nans(pts1, valid_mask1).reshape(len(pts1), -1, 3)
